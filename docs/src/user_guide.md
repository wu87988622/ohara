...
```bash
docker run --rm -p 12345:12345 oharastream/configurator:0.6-SNAPSHOT --port 12345
```
...
```bash
docker run --rm -p 5050:5050 oharastream/manager:0.6-SNAPSHOT --port 5050 --configurator http://$ip:12345/v0
```
...
1. oharastream/broker:0.6-SNAPSHOT
1. oharastream/zookeeper:0.6-SNAPSHOT
1. oharastream/connect-worker:0.6-SNAPSHOT
1. oharastream/configurator:0.6-SNAPSHOT
1. oharastream/manager:0.6-SNAPSHOT
1. oharastream/streamapp:0.6-SNAPSHOT
...
```sh
docker run --rm -p ${port}:${port} --add-host ${nodeHostName}:${nodeHostIP} oharastream/configurator:0.6-SNAPSHOT --port ${port} --hostname ${host} --node ${SshUserName}:${SshPassword}@${NodeHostName}:${SshPort}
```
...
$ docker run -v /tmp/configurator:/home/ohara/configurator \
         -p 12345:12345 \
         oharastream/configurator:0.6-SNAPSHOT \
         --port 12345 \
...
```sh
docker run --rm -p 5050:5050 oharastream/manager:0.6-SNAPSHOT --port 5050 --configurator http://localhost:12345/v0
```
...
docker pull oharastream/zookeeper:0.6-SNAPSHOT
docker pull oharastream/broker:0.6-SNAPSHOT
