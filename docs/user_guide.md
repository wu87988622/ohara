...
```bash
docker run --rm -p 12345:12345 oharastream/configurator:0.4.1 --port 12345
```
...
```bash
docker run --rm -p 5050:5050 oharastream/manager:0.4.1 --port 5050 --configurator http://$ip:12345/v0
```
...
1. oharastream/broker:0.4.1
1. oharastream/zookeeper:0.4.1
1. oharastream/connect-worker:0.4.1
1. oharastream/configurator:0.4.1
1. oharastream/manager:0.4.1
1. oharastream/streamapp:0.4.1
...
```sh
docker run --rm -p ${port}:${port} --add-host ${nodeHostName}:${nodeHostIP} oharastream/configurator:0.4.1 --port ${port} --hostname ${host} --node ${SshUserName}:${SshPassword}@${NodeHostName}:${SshPort}
```
...
```sh
docker run --rm -p 5050:5050 oharastream/manager:0.4.1 --port 5050 --configurator http://localhost:12345/v0
```
...
```h
docker run --rm -p 10000-10011:10000-10011 oharastream/backend:0.4.1 com.island.ohara.testing.service.FtpServer --controlPort 10000 --dataPorts 10001-10011 --user ${UserName} --password ${Password} --hostname ${hostIP or hostName}
```
