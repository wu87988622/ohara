...
object Workers {
  ...
  private[integration] def apply(workers: Option[String], brokers: => Brokers): Workers = workers
    ...
    .getOrElse {
      val validPorts = resolvePorts(NUMBER_OF_WORKERS)
      val connects = validPorts.zipWithIndex.map {
        case (port: Int, _: Int) =>
      ...
      new Workers {
        override def connectionProps: String = validPorts.map(p => s"${IoUtil.hostname}:$p").mkString(",")
        override protected def doClose(): Unit = {
