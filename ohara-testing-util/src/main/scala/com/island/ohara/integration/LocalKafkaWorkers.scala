...
/**
  ...
  */
class LocalKafkaWorkers(brokersConn: String, _ports: Seq[Int], baseProperties: Properties = new Properties) extends CloseOnce {
  ...
  ports.zipWithIndex.foreach { case (port: Int, index: Int) => {
    ...
    val herder = new DistributedHerder(distConfig, time, worker, statusBackingStore, configBackingStore, rest.advertisedUrl().toString())
