...
describe('Test schema restful api', () => {
  beforeEach(() => {
    moxios.install();
  });
  ...
  test('GET /api/schemas', async () => {
    ...
    expect(res.body.uuids['b2c07622-3288-4f68-8bc5-6a921cbfb863']).toBe("schema1");
  ...
  test('GET /api/schemas/{uuid}', async() => {
    ...
    expect(moxios.requests.mostRecent().url).toBe(CONFIGURATOR_GET_SCHEMA_DETAIL_URL);
  });
  ...
  test('POST /api/schemas', async() => {
    ...
    expect(moxios.requests.mostRecent().url).toBe(CONFIGURATOR_POST_CREATE_SCHEMA_URL);
  });
});
