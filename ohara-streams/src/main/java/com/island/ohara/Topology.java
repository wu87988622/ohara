...
public class Topology implements AutoCloseable {
    ...
    Topology(org.apache.kafka.streams.StreamsBuilder builder, org.apache.kafka.streams.StreamsConfig config, boolean isCleanStart) {
        this.topology = builder.build();

        //For now, windows handle cleanUp() -> DeleteFile(lock) with different behavior as Linux and MacOS
        ...
        if (isCleanStart) {
            final File baseDir = new File(config.getString(org.apache.kafka.streams.StreamsConfig.STATE_DIR_CONFIG));
            final File stateDir = new File(baseDir, config.getString(org.apache.kafka.streams.StreamsConfig.APPLICATION_ID_CONFIG));
            try {
    ...
    public String describe() {
        topology.describe().subtopologies().forEach(subtopology -> {
