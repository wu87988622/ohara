...
class TestNameLimitOfCluster extends SmallTest with Matchers {
  private[this] implicit val nodes: NodeCollie = NodeCollie.inMemory(Seq.empty)
  private[this] val collie = new ClusterCollieImpl
  ...
  @After
  def tearDown(): Unit = Releasable.close(collie)
}
