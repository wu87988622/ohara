...
class TestStreamRoute extends SmallTest with Matchers {
  ...
  def testOnlyAcceptOneTopic(): Unit = {
    ...
    val t1 = topicKey()
    val t2 = topicKey()
    result(topicApi.request.key(t1).create().flatMap(_ => topicApi.start(t1)))
    result(topicApi.request.key(t2).create().flatMap(_ => topicApi.start(t2)))
    val thrown1 = the[IllegalArgumentException] thrownBy result(
      accessStream.request
        .name(streamDesc.name)
        .fromTopicKey(t1)
        .toTopicKeys(Set(t1, t2))
        .update()
