...
private[configurator] object ValidationRoute extends SprayJsonSupport {
  ...
  def apply(implicit brokerClient: BrokerClient, connectClient: WorkerClient): server.Route =
    pathPrefix(VALIDATION_PREFIX_PATH) {
      verifyRoute(
        root = VALIDATION_HDFS_PREFIX_PATH,
        verify =
          (req: HdfsValidationRequest) => Validator.run(connectClient, brokerClient, req, DEFAULT_NUMBER_OF_VALIDATION)
      ) ~ verifyRoute(
        root = VALIDATION_RDB_PREFIX_PATH,
        verify =
          (req: RdbValidationRequest) => Validator.run(connectClient, brokerClient, req, DEFAULT_NUMBER_OF_VALIDATION)
      ) ~ verifyRoute(
        root = VALIDATION_FTP_PREFIX_PATH,
        verify =
          (req: FtpValidationRequest) => Validator.run(connectClient, brokerClient, req, DEFAULT_NUMBER_OF_VALIDATION)
      ) ~ verifyRoute(
